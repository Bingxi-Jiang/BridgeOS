...
const els = Object.fromEntries([
  ...
  "peopleDirectory", "conversationList", "peopleList", "mapView", "todayView", "peopleView", "conversationsView"
...
function closeOverlays() {
  [els.captureDrawer, els.detailDrawer].forEach(el => { el.classList.remove("open"); el.setAttribute("aria-hidden", "true"); });
...
function draft(type, id) {
  ...
  if (type === "intro") {
    ...
    evidence = `Grounded in ${p.memories} memories with ${p.name}`;
  }
...
function answerQuestion(question) {
  ...
  let items = [];
  let evidence = [];
  if (q.includes("software") || q.includes("swe") || q.includes("opportunit") || q.includes("remote")) {
    ...
    title = "Two follow-ups are worth doing tonight";
    lead = "Theo is the most time-sensitive because you explicitly promised a demo. Lena is the highest-upside conversation to continue before the booths close.";
    items = [["Theo Brooks", "Send the 30-second interruption demo by 9:00 PM", "theo"], ["Lena Ortiz", "Ask for the streaming architecture review she offered", "lena"]];
    evidence = ["2 open commitments", "Event schedule · booths close 10 PM"];
    ...
    evidence = ["8 conversations", "6 people", "14 linked topics"];
  }
...
document.querySelectorAll("[data-capture-mode]").forEach(button => button.addEventListener("click", () => {
  document.querySelectorAll("[data-capture-mode]").forEach(b => b.classList.toggle("active", b === button));
...
document.getElementById("finishCapture").addEventListener("click", () => { closeOverlays(); toast("Amara added · 3 connections updated"); });
